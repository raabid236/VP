...
geometry = [4 4 8];
% load homographies
seq3homo = [pathIm, 'Homographies'];
load(fullfile(pathIm, [seq3homo, '.mat']) );
allHomographies = eval(seq3homo);
clear(seq3homo);
...
for imageIndex =1:numOfTransforms
    ...
    n_matches = size(matchPos,2);

    % get matched locations of keypoints, transpose
    loc1XY = im1l(:,matchPos(1,:))';
    loc2XY = im2l(:,matchPos(2,:))';
    % center points
    loc1XYC = loc1XY - repmat(im1SizeXYHalf,  n_matches, 1);
	loc2XYC = loc2XY - repmat(im1SizeXYHalf,  n_matches, 1);
    
    current_H = allHomographies(imageIndex).H;
    % estimate distances
    e_dist = get_dist(loc1XYC, loc2XYC, current_H);
    
    % save estimated distances
    e_dist_all{imageIndex} = e_dist;
    
    %disp(sprintf('Average dist orig: %.2f', mean(e_dist)));
